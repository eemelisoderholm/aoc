...
<span title="Turtles, all the way down.">programs</span> that have gotten
themselves into a bit of trouble. A recursive algorithm has gotten out of hand,
...
_their_ own disc, and so on. At the very tops of these sub-sub-sub-...-towers,
...
balanced. Apparently, one program has the _wrong weight_, and until it's fixed,
they're stuck here.
...
_and all programs above it_ must each match. This means that the following sums
...
Given that exactly one program is the wrong weight, _what would its weight need
to be_ to balance the entire tower?
