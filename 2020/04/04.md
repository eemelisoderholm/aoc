...
_detecting which passports have all required fields_. The expected fields are as
follows:
...
According to the above rules, your improved system would report _`2`_ valid
passports.
...
<span title="GLORY TO ARSTOTZKA">strict rules</span> about what values are valid
...
Your job is to count the passports where all required fields are both _present_
and _valid_ according to the above rules. Here are some example values:
...
Count the number of _valid_ passports - those that have all required fields _and
valid values_. Continue to treat `cid` as optional. _In your batch file, how
many passports are valid?_
