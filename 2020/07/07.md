...
due to _issues in luggage processing_.
...
You have a _`shiny gold`_ bag. If you wanted to carry it in at least one other
...
least one `shiny gold` bag is _`4`_.

_How many bag colors can eventually contain at least one `shiny gold` bag?_ (The
...
within it) plus 2 `vibrant plum` bags (and the 11 bags within _each_ of those):
`1 + 1*7 + 2 + 2*11` = _`32`_ bags!

Of course, the actual rules have a <span title="100%">small</span> chance of
going several levels deeper than this example; be sure to count all of the bags,
